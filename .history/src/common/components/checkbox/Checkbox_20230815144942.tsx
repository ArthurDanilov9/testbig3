...
import { useController, Control, useFormReturn } from 'react-hook-form';
...
interface ICheckbox {
  name: string;
  label: string;
  control: Control<ISignUpConfirm>;
}
...
const Checkbox: FC<ICheckbox> = ({ name, label, control }) => {
  ...
  return (
    <label
      className={error ? `${styles.checkbox} ${styles.checkboxRequired}` : styles.checkbox}
      onClick={handleCheckboxClick}>
      <div
        className={
          error ? styles.customCheckRequired : value ? styles.customCheckActive : styles.customCheck
        }>
        {!value ? (
          <svg
      ...
      <input
        ...
        type="checkbox"
      />
      <p className={error ? styles.labelRequired : ''}>{label}</p>
