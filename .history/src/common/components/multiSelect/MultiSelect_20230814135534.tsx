...
import style from './multiSelect.module.sass';
...
const MultiSelect: FC<IMultiSelect> = ({ options, setTeamIds }) => {
  ...
  const handleSelectChange = (newValue: MultiValue<IOption>, actionMeta: ActionMeta<IOption>) => {
    setTeamIds(newValue.map((option) => option.id));
    setTotalCount(newValue.length);
  };
  ...
  return (
    <div className={totalCount > 2 ? `${style.container} ${style.after}` : style.container}>
      <Select
        styles={styles}
        options={options}
