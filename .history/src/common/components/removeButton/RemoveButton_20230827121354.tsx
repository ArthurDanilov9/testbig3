  ...
  const deleteItem = async () => {
    if (id) {
      if (location.pathname.includes('/players')) {
        const deleteResponse = await dispatch(deletePlayer(+id));
        if (deleteResponse.payload && !('status' in deleteResponse.payload)) {
          dispatch(setAlert({ showed: true, message: 'Player was deleted', type: 'success' }));
          navigate('/players');
        }
      } else if (location.pathname.includes('/teams')) {
        const deleteResponse = await dispatch(deleteTeam(+id));
        if (deleteResponse.payload && !('status' in deleteResponse.payload)) {
