    ...
    if (data.foundationYear) {
      if (data.foundationYear > 2023) {
        methods.setError('foundationYear', {
          type: 'manual',
          message: `You cannot create a team with a year greater than the current year!`,
        });
        return;
  ...
  return (
    <FormProvider {...methods}>
      <form className={styles.form} onSubmit={methods.handleSubmit(submitHandler)}>
        ...
        {showAlert && (
          <Alert isSuccess={false} message={errorMessage} setShow={setShowAlert} show={showAlert} />
        )}
      </form>
    </FormProvider>
  );
